...
| 2026-03-31 | Pantalla Blanca "White Screen of Death" en producción (Vercel) al acceder al sitio completo. | Variables de entorno `VITE_SUPABASE_URL` y `VITE_SUPABASE_ANON_KEY` faltantes en Vercel causaron que `src/lib/supabase.ts` lanzara un error crítico (`throw new Error`) durante la evaluación del módulo raíz, bloqueando que React renderizara cualquier cosa. | **Solución:** Usar Fallbacks (`|| FALLBACK_URL`) explícitos en el código para evitar que el JS colapse, o configurar las variables de entorno en el Dashboard de Vercel (Recomendado). Adicionalmente, atrapar promesas no manejadas (`.catch`) de `getSession()` si el backend se encuentra pausado (estado INACTIVE). |
| 2026-04-08 | Error en Frontend: "Database error saving new user" al crear cuenta en Step 2 (Company y Employee). | Un trigger en PostgreSQL (`sync_profile_email` y otros en schema `auth`) omitía el schema `public.` al referenciar la tabla `profiles`. Al estar bajo `SECURITY DEFINER` o en el schema `auth`, PostgreSQL devuelve _relation "profiles" does not exist_ cancelando la transacción entera del `signUp`. | **Solución:** Reemplazar y calificar SIEMPRE las tablas en triggers de la base de datos de Supabase como `public.profiles` (`UPDATE public.profiles SET...`), nunca dejarlas sin prefijo. |
